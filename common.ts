...
import { readableStreamToString, VisierDocumentSearchError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierDocumentSearchError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierDocumentSearchError(e, e.response?.data)
                }
                if (innerError instanceof VisierDocumentSearchError) {
                    // Got "VisierDocumentSearchError" from the above try block
                    throw innerError;
                }
